...
if [ "$OPEN_SHORT_TAG" = true ] ; then
    sed -i -r 's/short_open_tag = Off$/short_open_tag = On/' /etc/php/7.2/apache2/php.ini
    sed -i -r 's/short_open_tag = Off$/short_open_tag = On/' /etc/php/7.2/cli/php.ini
fi
if [ "$OPEN_SHORT_TAG" = false ] ; then
    sed -i -r 's/short_open_tag = On$/short_open_tag = Off/' /etc/php/7.2/apache2/php.ini
    sed -i -r 's/short_open_tag = On$/short_open_tag = Off/' /etc/php/7.2/cli/php.ini
fi

# Configuring PHP display_errors
DISPLAY_ERRORS=${ERRORS:-false}
if [ "$DISPLAY_ERRORS" = true ] ; then
    sed -i -r 's/display_errors = Off$/display_errors = On/' /etc/php/7.2/apache2/php.ini
    sed -i -r 's/display_errors = Off$/display_errors = On/' /etc/php/7.2/cli/php.ini
fi
if [ "$DISPLAY_ERRORS" = false ] ; then
    sed -i -r 's/display_errors = On$/display_errors = Off/' /etc/php/7.2/apache2/php.ini
    sed -i -r 's/display_errors = On$/display_errors = Off/' /etc/php/7.2/cli/php.ini
fi
